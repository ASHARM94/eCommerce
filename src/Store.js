...
function Store() {
    ...
    let [showAddedToCartAlert, setshowAddedToCartAlert] = useState(false);
    ...
    useEffect(() => {
        (async () => {
            //get brands from db
            let brandsResponse = await fetch("http://localhost:5000/brands",
                {
                    method: 'GET',
                });
            ...
            let categoriesResponse = await fetch("http://localhost:5000/categories",
                {
                    method: 'GET',
                });
